...
After the config message is received the client needs to respond back with 1 byte of data (doesn't matter the value) and then will receive the amplitudes in dBFS encoded as a 32 bit float bytearray having a length defined by the frequency band count received in the configuration message. The server will send each amplitude set every (1 / sample rate) seconds
